...
4. **Clone + pull, then symlink one file out of the clone** (Spaceship) — the theme is an upstream git repo that must be *updated in place*, not a single file this repo vendors, so strategy 1 has nothing to point at. `setup_spaceship` clones `--depth=1` (or `git pull --ff-only` if already there), then uses `link()` only on `spaceship.zsh-theme`, surfacing it one level up because oh-my-zsh discovers themes only directly in the themes dir. **Never call `link()` on the clone directory**: it backs up and replaces its destination, so every re-run would move the clone aside and re-clone, littering `$HOME` with `.backup.*` copies.
