...
- **`claude/skills/plan-artifacts` bundles its own templates.** The skill's prompt originally read `~/.config/claude-templates/{UI_TEMPLATE.html,TODO_TEMPLATE.md}`, which does not exist on any machine here; the templates therefore live in `claude/skills/plan-artifacts/templates/` and are tracked, so the skill is self-contained and a per-skill directory symlink carries them along. The `~/.config` path is kept as a documented *override*, not a dependency — the skill must still work when it is absent.
