...
4. **Untracked runtime state** (the statusline colour DB) — a file a *tracked script* creates on the machine, which is deliberately neither tracked nor linked. `~/.claude/statusline-colors.db` holds the per-session colour assignments; it is machine-local by definition (a colour belongs to this laptop's set of checkouts) and would be meaningless on another machine. Nothing in `setup.sh` creates it — `claude/lib/session-colors.sh` does, on first use, with `CREATE TABLE IF NOT EXISTS`. The thing to get right when adding another of these is that its *absence must be a working state*, not an error: the statusline falls back to hashing when the DB cannot be opened.
...
- **The statusline's colour comes from a database, not from a hash.** `claude/lib/session-colors.sh` records a colour per directory+branch in `~/.claude/statusline-colors.db` and **never reassigns an existing row** — that permanence is the feature, not an optimisation, so anything that could repaint a live session mid-work is a bug. Three details are load-bearing and each has a regression test that was mutation-checked:
  - `PRAGMA busy_timeout=10000` — without it a locked write is abandoned *silently* (11 of 24 parallel rows lost, zero stderr) or prints `database is locked`, which discards the whole statusline. The test therefore asserts on **rows written**; a stderr-only assertion passes vacuously. The ceiling is a backstop against a machine stall, not a contention budget — real waits are sub-millisecond.
  - `INSERT OR IGNORE` — `OR REPLACE` lets the loser of a race overwrite the winner and repaint a live session.
  - The `SESSION_COLOR_PALETTE` **order** — assignment walks it, so adjacent entries go to sessions likely to be open together. It is tuned for maximum adjacent CIELab distance (min ΔE 91.3) and must be regenerated, not hand-edited.

  **The palette is defined in exactly one place** — `SESSION_COLOR_PALETTE` in `claude/lib/session-colors.sh`. `statusline.sh` sources the library before it needs a colour, so the hash fallback indexes that same array; it keeps only a tiny guarded default for the case where the library cannot be read at all, which is deliberately *not* a copy of the real list (a stale duplicate that silently disagreed would be worse than an obviously reduced one). Changing the palette — its contents **or its length** — is a one-line edit in that one file: the tests derive their expectations from the array rather than restating it, so nothing else needs touching. Two tests pin this: one fails if a second `PALETTE=(...)` reappears in `statusline.sh` or the indexed read is renamed away, the other checks every code in the guarded default still clears the contrast bar.

  **Scoring the palette requires the terminal theme's real foregrounds**, not xterm's defaults — `#d9bd26` / `#1cd915` under ghostty `deep`. Getting this wrong is not hypothetical: it shipped 7 of 16 codes below the 3.0 bar, because the swatch page used to pick them rendered colours the terminal never draws. Codes `0`–`15` are theme-resolved too, so they are **not** on the 6×6×6 cube and must not be mapped through it.

  Retention and the DB path are tunables in `claude/statusline.conf`, live via the symlink.
- **The context gauge is scaled to the auto-compact point, not to the raw window.** Claude Code compacts once input tokens reach `CTX_MAX - CTX_RESERVE` (167,000 of 200,000), because the reserve is an *absolute* output budget rather than a fraction of the window — the 1M-window case compacts at ~967K, the same ~33,000 held back. Dividing by `CTX_MAX` made the bar read ~84% at the instant compaction fired. `CTX_RESERVE=33000` is **empirical, not documented by Anthropic**: measured across 93 `compact_boundary` records, `200000 - preTokens` fell in a 32,852–33,273 band. The conf file carries the `jq` one-liner to re-measure it if compaction drifts.

  Both constants live in `claude/context-window.conf`, **not** `statusline.conf`. The split is deliberate: `statusline.conf` holds that script's *display* tunables, whereas these describe when *Claude Code itself* compacts — the statusline only consumes them, and any other hook or script needing the threshold should read the same file. `statusline.sh` sources it directly (not by way of `session-colors.sh`, which is sourced far later and skipped when unreadable) and defaults both values inline, so an absent or half-written conf still renders. Adding it to the flat-file list in `setup_claude` is what installs it.

  The token count and window size now come from the payload's `context_window` object (`total_input_tokens`, `context_window_size`); the transcript sum is the fallback for older CLIs and for the null window at session start. `context_window.used_percentage` is deliberately **not** used — it divides by the raw window and reads ~83.5% at compaction, which is the bug. `context_window_size` is clamped *down* to `CTX_MAX`, since `autoCompactWindow` can cap the effective window below the model's native one.
...
- **`claude/skills/plan-artifacts` bundles its own templates.** The skill's prompt originally read `~/.config/claude-templates/{UI_TEMPLATE.html,TODO_TEMPLATE.md}`, which does not exist on any machine here; the templates therefore live in `claude/skills/plan-artifacts/assets/` and are tracked, so the skill is self-contained and a per-skill directory symlink carries them along. `assets/` is the name Claude Code's own skill layout reserves for templates and fonts — the sibling conventions are `scripts/` for executables and `references/` for docs loaded on demand, which is why `clean-code` keeps its chapters under `references/`. The `~/.config` path is kept as a documented *override*, not a dependency — the skill must still work when it is absent.
