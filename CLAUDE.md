...
- **The statusline's colour comes from a database, not from a hash.** `claude/lib/session-colors.sh` records a colour per directory+branch in `~/.claude/statusline-colors.db` and **never reassigns an existing row** — that permanence is the feature, not an optimisation, so anything that could repaint a live session mid-work is a bug. Three details are load-bearing and each has a regression test that was mutation-checked:
  ...
  - The `SESSION_COLOR_PALETTE` **order** — assignment walks it, so adjacent entries go to sessions likely to be open together. It is tuned for maximum adjacent CIELab distance (min ΔE 90) and must be regenerated, not hand-edited.

  The palette is duplicated in `claude/scripts/statusline.sh` (as `PALETTE`, for the fallback path); a test asserts the two agree. Retention and the DB path are tunables in `claude/statusline.conf`, live via the symlink.
