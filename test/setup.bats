...
@test "no document states a palette size that disagrees with the array" {
  # The README tells the user when colours start repeating, and the palette is
  # the only source for that number. A stale one is worse than no number.
  local n root
  ...
  bats_run grep -nE "(Past|all) ${n}\b" "${root}/README.md"
  [ "$status" -eq 0 ]

  # Phrasing-independent: any quantifier or copula binding a count to the
  # palette, rather than a bare co-occurrence of a number and the word. Matches
  # are then filtered down to those stating a size OTHER than the real one, so
  # this catches a rewording as well as a stale number.
  bats_run grep -nhEoi \
    "(past|all|only|the) [0-9]+ (colours?|codes?|keys|entries|assignments)|(palette|list) (of|has|holds|contains) [0-9]+" \
    "${root}/README.md" "${root}/claude/lib/session-colors.sh"
  bats_run grep -vE "(^| )${n}( |\$)" <<<"$output"
  [ -z "$output" ]
}
...
# --- prompt spec -----------------------------------------------------------
#
# The expectation files ARE the spec. Nothing here executes agnoster: the
# prompt owns its own appearance and must stay free to diverge from the theme
# it was originally derived from.

PROMPT_STATES="non-git git-clean git-dirty detached-head exit-nonzero jobs root venv conda"
...
@test "the prompt spec does not depend on the agnoster theme being present" {
  ...
  mv "$theme" "$moved"
  bats_run "${BATS_TEST_DIRNAME}/check-prompt.sh" git-clean
  mv "$moved" "$theme"
...
@test "check-prompt reports a mismatch and names the differing segment" {
  ...
  mkdir -p "$tmp"
  sed 's/bg=2/bg=6/' "$dir/git-clean" >"$tmp/git-clean"

  PROMPT_EXPECTED_DIR="$tmp" bats_run "${BATS_TEST_DIRNAME}/check-prompt.sh" git-clean
  [ "$status" -ne 0 ]
